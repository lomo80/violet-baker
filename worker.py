...
for ts in timestamps:
    ...
    # [3.5] 압축 전 내부 청소 및 이름 통일
    # 1. db 파일 이름을 data.db로 바꿉니다.
    os.rename(f"dl_master/data-{ts}.db", "dl_master/data.db")
    
    # 2. 쓸모없는 유령 파일(release_info_*.txt)을 삭제해서 압축에 안 들어가게 막습니다.
    info_file = f"dl_master/release_info_{ts}.txt"
    if os.path.exists(info_file):
        os.remove(info_file)

    # [4] 7z 압축 (초핵심!)
    print(f"☁️ {ts} 마스터 데이터 7z 압축 중...")
    # dl_master 폴더 '안으로' 들어가서(cwd='./dl_master') 내부 파일만(*) 압축합니다! 
    subprocess.run(
        ['7z', 'a', '-t7z', '../rawdata.7z', '*'], 
        cwd='./dl_master', 
        check=True, 
